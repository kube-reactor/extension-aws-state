...
function destroy_remote_state_aws_s3 () {
  ...
  if [[ "${REACTOR_FORCE_STATE_UPDATE:-}" ]] || [[ "$AWS_STATE_KMS_KEY_ID" ]]; then
    for access_key_id in $(aws iam list-access-keys --user-name "$AWS_PLATFORM_WRITE_USER" --query "AccessKeyMetadata[].AccessKeyId" --output "text" 2>/dev/null); do
      aws iam delete-access-key --user-name "$AWS_PLATFORM_WRITE_USER" --access-key-id "$access_key_id"
    done
    for access_key_id in $(aws iam list-access-keys --user-name "$AWS_CONTAINER_WRITE_USER" --query "AccessKeyMetadata[].AccessKeyId" --output "text" 2>/dev/null); do
      aws iam delete-access-key --user-name "$AWS_CONTAINER_WRITE_USER" --access-key-id "$access_key_id"
    done
    for access_key_id in $(aws iam list-access-keys --user-name "$AWS_CONTAINER_READ_USER" --query "AccessKeyMetadata[].AccessKeyId" --output "text" 2>/dev/null); do
